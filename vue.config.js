...
module.exports = {
  ...
  pluginOptions: {
    ssr: {
      ...
      nodeExternalsWhitelist: [/\.css$/, /\?vue&type=style/, 'lodash-es', /^@aeternity\/aepp-sdk\/es/],
    },
  },
  css: {
    loaderOptions: {
  ...
  chainWebpack: (config) => {
    config
      ...
      .tap((options) => {
        ...
        definitions['process.env.VUE_CLI_SSR'] = JSON.stringify(process.env.VUE_CLI_SSR_TARGET === 'server');
        return [definitions];
      })
      .end()
